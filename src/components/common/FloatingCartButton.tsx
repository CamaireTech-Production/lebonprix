    ...
    <>
      ...
      <div className={`fixed bottom-20 right-4 z-40 ${className}`}>
        <button
          ...
          className={`relative text-white p-4 rounded-full shadow-lg transition-all duration-200 hover:scale-105 ${showAddedAnimation ? 'animate-bounce scale-110' : ''
            }`}
        ...
        >
          ...
          {getCartItemCount() > 0 && (
            <div className={`absolute -top-2 -right-2 bg-theme-brown text-white text-xs rounded-full w-6 h-6 flex items-center justify-center font-semibold ${showAddedAnimation ? 'animate-pulse' : ''
              }`}>
              {getCartItemCount()}
            </div>
          )}
