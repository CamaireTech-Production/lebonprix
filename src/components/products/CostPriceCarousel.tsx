...
const CostPriceCarousel: React.FC<CostPriceCarouselProps> = ({ batches, className = '' }) => {
  ...
  // Filter batches with status 'active' and remaining quantity > 0, then sort by creation date (newest first)
  const activeBatches = batches
    .filter(batch => batch.status === 'active' && batch.remainingQuantity > 0)
    .sort((a, b) => {
