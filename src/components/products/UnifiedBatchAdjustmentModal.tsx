...
import type { Product, StockBatch, BatchAdjustment } from '../../types/models';
...
const UnifiedBatchAdjustmentModal: React.FC<UnifiedBatchAdjustmentModalProps> = ({
      ...
      case 'quantity_correction':
        ...
        if (!newTotalQtyStr || newTotalQtyStr.trim() === '') {
          ...
          if (isNaN(newTotal) || newTotal < 0) {
            ...
            const minAllowed = usedQuantity; // Can't be less than what's already used
  ...
  const calculateImpact = () => {
    ...
    let newCost = selectedBatch.costPrice;
    let stockChange = 0;
    ...
    switch (adjustmentType) {
      ...
      case 'cost_correction':
        if (getStringValue(formData.newCostPrice).trim() !== '') {
          newCost = parseFloat(getStringValue(formData.newCostPrice)) || selectedBatch.costPrice;
        }
        break;
    ...
    return {
      ...
      currentCost: selectedBatch.costPrice
    };
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await adjustBatchUnified(product.id, 'product', adjustment, company.id);
      
      showSuccessToast('Batch adjustment recorded successfully!');
      onSuccess?.();
      onClose();
    } catch (error) {
  ...
  const batchOptions = batches.map(batch => {
    ...
    return {
      value: batch.id,
      label: `Batch ${batch.id.slice(-8)} - ${batch.remainingQuantity}/${batch.quantity} @ ${formatCostPrice(batch.costPrice)} (${locationInfo})`
    };
  });
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Ajustement de lot - ${product.name}`}
      size="lg"
    >
        ...
        {selectedBatch && (
          <>
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium">{formatCostPrice(selectedBatch.costPrice)}</p>
                </div>
                <div>
            ...
            {(adjustmentType === 'remaining_adjustment' || adjustmentType === 'combined') && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Ajustement du stock restant (delta)
                </label>
                <Input
                  type="number"
              ...
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium">{impact.currentRemaining}/{impact.currentTotal} @ {formatCostPrice(impact.currentCost)}</p>
                  </div>
                  <div>
                    <span className="text-blue-700">Après:</span>
                    <p className="font-medium">{impact.newRemaining}/{impact.newTotal} @ {formatCostPrice(impact.newCost)}</p>
                  </div>
                </div>
                {impact.stockChange !== 0 && (
                  <div className="mt-2 pt-2 border-t border-blue-200">
                    ...
                    <p className={`font-medium ${impact.stockChange > 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {impact.stockChange > 0 ? '+' : ''}{impact.stockChange}
                    </p>
                  </div>
                )}
              </div>
