...
export class SecureEncryption {
  ...
  static verifyHmacSignature(payload: string, signature: string, secret: string): boolean {
    try {
      ...
      // CryptoJS doesn't have timingSafeEqual, so we use a simple comparison
      // In production, consider using a library that provides constant-time comparison
      const signatureHex = CryptoJS.enc.Hex.parse(signature);
      const expectedHex = CryptoJS.enc.Hex.parse(expectedSignature);
      
      // Simple constant-time comparison
      if (signatureHex.words.length !== expectedHex.words.length) {
        return false;
      }
      
      let result = 0;
      for (let i = 0; i < signatureHex.words.length; i++) {
        result |= signatureHex.words[i] ^ expectedHex.words[i];
      }
