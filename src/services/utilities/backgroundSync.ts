...
class BackgroundSyncService {
  ...
  static async syncProducts(companyId: string, onUpdate?: (products: Product[]) => void): Promise<void> {
    ...
    try {
      ...
      const unsubscribe = subscribeToProducts(companyId, (freshProducts: Product[]) => {
  ...
  static async syncSales(userId: string, onUpdate?: (sales: Sale[]) => void): Promise<void> {
    ...
    try {
      ...
      // Subscribe to Firebase for fresh data
      const unsubscribe = subscribeToSales(userId, (freshSales: Sale[]) => {
  ...
  static async syncExpenses(userId: string, onUpdate?: (expenses: Expense[]) => void): Promise<void> {
    ...
    try {
      ...
      // Subscribe to Firebase for fresh data
      const unsubscribe = subscribeToExpenses(userId, (freshExpenses: Expense[]) => {
