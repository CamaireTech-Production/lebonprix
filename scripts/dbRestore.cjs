...
/**
 * Restaurer les sous-collections employeeRefs
 */
async function restoreEmployeeRefs(companyId, employeeRefs, options) {
  if (!employeeRefs || employeeRefs.length === 0) return;

  console.log(`👥 Restauration des références employés pour l'entreprise ${companyId}`);
  
  if (options.dryRun) {
    console.log(`   [DRY-RUN] ${employeeRefs.length} références employés seraient restaurées`);
    return { success: true, count: employeeRefs.length };
  }
  ...
  for (const employeeRef of employeeRefs) {
    try {
      const empRefDoc = db.collection('companies').doc(companyId).collection('employeeRefs').doc(employeeRef.id);
      batch.set(empRefDoc, employeeRef);
      successCount++;
    } catch (error) {
      console.error(`❌ Erreur référence employé ${employeeRef.id}: ${error.message}`);
    }
  }

  try {
    await batch.commit();
    console.log(`✅ Références employés ${companyId}: ${successCount} documents restaurés`);
    return { success: true, count: successCount };
  } catch (error) {
    console.error(`❌ Erreur batch références employés ${companyId}: ${error.message}`);
    return { success: false, count: 0 };
  }
}
  ...
  try {
    ...
    for (const collectionName of collectionsToRestore) {
      try {
        ...
        // Restaurer les sous-collections pour companies
        if (collectionName === 'companies' && result.success) {
          for (const company of data) {
            // Restaurer la sous-collection employees (ancienne structure)
            if (company.employees && company.employees.length > 0) {
              await restoreEmployees(company.id, company.employees, options);
            }
            
            // Restaurer la sous-collection employeeRefs (nouvelle structure)
            if (company.employeeRefs && company.employeeRefs.length > 0) {
              await restoreEmployeeRefs(company.id, company.employeeRefs, options);
            }
          }
        }
      } catch (error) {
